...
let correo = document.querySelectorAll('.search-input')[1].value.trim();
...
async function obtenerPublicacionesFiltradas() {
  try {
    ...
    correo = document.querySelectorAll('.search-input')[1].value.trim();
    fecha = document.getElementById('filter-date').value;
    tema = document.getElementById('filter-topic').value;

    const response = await fetch('/api/publicaciones_filtradas', {
      method: 'POST',
      headers: {
